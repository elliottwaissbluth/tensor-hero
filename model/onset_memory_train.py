...
from tensor_hero.model import ContourOnsetMemoryDataset, OnsetTransformer
import torch
...
def main():
        ...
        params = {
            ...
            'train_path' : str(Path.cwd() / 'Training_Data' / 'training_ready' / 'train'),
            ...
            'max_trg_len' : 103, # NOTE: max_trg_len <= max_src_len otherwise side asset error is triggered
            'max_src_len' : 103,
            'trg_vocab_size' : 25,
            'pad_idx' : 2,
            'embedding_size' : 512,
        ...
        # Define data loaders
        train_data = ContourOnsetMemoryDataset(Path(params['train_path']), 
                                        params['max_src_len'], 
                                        params['max_trg_len'],
                                        max_examples,
                                        params['pad_idx'])
